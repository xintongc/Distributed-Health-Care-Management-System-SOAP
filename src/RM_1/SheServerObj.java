...
public class SheServerObj implements ServerInterface, Runnable {
    ...
    private static HashMap<String, SheBookingDb> users = new HashMap<String, SheBookingDb>();
    ...
    private SheBookingDb patientID;


    public SheServerObj() {
    	docMap.put("SHEA130619", 1);
    	//docMap.put("SHEE110619", 1);
//    	docMap.put("SHEE110619", 3);
//    	docMap.put("SHEA111219", 3);
//    	docMap.put("SHEA151219", 3);
//        docMap.put("SHEA101219", 3);
//        dentMap.put("SHEA010919", 3);
        dentMap.put("SHEA250619", 1);
       // surgMap.put("SHEM041219", 3);
        adminUsers.put("SHEA1111", "");
        Thread t1 = new Thread(this);
        t1.setName("This is Que default thread");
        t1.start();
        this.patientID = null;
        try {
            file = new FileWriter("SheServer.txt", true);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void setSurgMap(HashMap<String, Integer> surgMap) {
        SheServerObj.surgMap = surgMap;
    }

    public static HashMap<String, SheBookingDb> getUsers() {
        return users;
    }

    public static void setUsers(HashMap<String, SheBookingDb> users) {
        SheServerObj.users = users;
    }
    ...
    public synchronized String bookAppointment(String patientID, String appointmentID, String appointmentType) {
        ...
        if (!users.containsKey(patientID)) {
            this.patientID = new SheBookingDb(patientID);
        }
        String patLoc= patientID.substring(0,3);
        System.out.println(patLoc);
    ...
    public String getAppointmentSchedule(String patientID){
        ...
        System.out.println("Inside server app map " + this.patientID.getAppMap());
        for (String key : this.patientID.getAppMap().keySet()) {
            out = out + this.patientID.getAppMap().get(key) + "-" + key + "\n";

        }
        System.out.println("in server appshad responce final " + out);
    ...
    public String showSlots(String appointmentType) {
        ...
        if (appointmentType.equals("Physician")) {
            return ("The following Physician appointments are available in Sherbrooke:\n" + docMap.keySet());
        }
        if (appointmentType.equals("Dentist")) {
            return ("The following Dentist appointments are available in Sherbrooke:\n" + dentMap.keySet());
        }
        if (appointmentType.equals("Surgeon")) {
            return ("The following Surgeon appointments are available in Sherbrooke:\n" + surgMap.keySet());
        }
        return ("There are no appointments slots available in Sherbrooke");
    }

    public boolean appEx(String appointmentID, String appointmentType) {
        System.out.println("in appEx ::" + appointmentID + "_" + appointmentType);

        if (appointmentType.equals("Physician") && docMap.containsKey(appointmentID)) {

            return (true);
        }
        if (appointmentType.equals("Dentist") && dentMap.containsKey(appointmentID)) {

            return (true);
        }
        if (appointmentType.equals("Surgeon") && surgMap.containsKey(appointmentID)) {
...
//    }
            ...
            if (docMap.containsKey(appointmentID)) {
                ...
                for (String userKey : users.keySet()) {
                    for (String appKey : users.get(userKey).getAppMap().keySet()) {
                        System.out.println("THIS HERE " + appKey);
    ...
    public String listAppointmentAvailability(String appointmentType){
        ...
        //System.out.println("Just before final rmi call listAppointmentAvailability :" + retMsg + "\n" + recvData(appointmentType)+ "llllllllllllllllll");
        ...
        try {
            ...
            while (true) {
                ...
                if (inMsg.startsWith("Physician")) {
                    System.out.println("Match Physician ");
                ...
                if (inMsg.startsWith("Dentist")) {
                    System.out.println("Match Dentist ");
                ...
                if (inMsg.startsWith("Surgeon")) {
                    System.out.println("Match Surgeon ");
                ...
                if (inMsg.startsWith("$")) {
                    System.out.println("inside  server if(send) ");
...
///send and receive reply from Quebec
            ...
            retMsg = ("#Available " + appointmentType + " appointments in Quebec:\n" + temp);
///send and receive reply from Quebec
...
//                }
    ...
    public boolean isBookableExt(String patLoc, String ID, String type, String patID) {
        ...
        System.out.println("IsBookable SHE =:\n" +
                "MTL = " +mtl +
                "\nSHE = " +she+
                "\nQUE = "+ que);
    ...
    public synchronized boolean swapAppointment(String patientID, String oldAppointmentID, String oldAppointmentType, String newAppointmentID, String newAppointmentType) {
        ...
        if (oldLoc.equals("SHE")) {
            ...
            if (newLoc.equals("QUE") || newLoc.equals("MTL")) {
                try {
                    aSocket = new DatagramSocket();
                    InetAddress aHost = InetAddress.getByName("localhost");
